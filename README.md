...
这是一个功能完整的企业级多币种资金池自动化管理系统，包含**完整的Web管理界面**和后台自动化脚本：

### 核心功能
- 📊 **数据仪表盘**: 实时显示各币种余额、资金归集率、汇率波动概览
- 📝 **交易流水查询**: 多维度组合查询，支持分页和批量导出（Excel/CSV/JSON）
- 📈 **资金预测看板**: 7天各币种资金预测曲线，缺口自动高亮，点击查看换汇方案
- ✅ **换汇审批中心**: 待审批方案列表，显示最优路径和成本对比，支持多级审批
- ✏️ **手动调整录入**: 大额预付/延期付款表单，提交后自动更新预测
- ⚠️ **风险预警中心**: 连续3天波动超2%自动预警，推送对冲建议
- 📑 **月度报告管理**: 报告列表展示，支持PDF和Excel下载
- 多银行账户实时余额和交易流水抓取
- 最优换汇路径计算和成本对比
...
Web界面包含7个功能模块：
1. **📊 数据仪表盘** - 资金总览、余额卡片、汇率行情
2. **📝 交易流水查询** - 多条件筛选、分页、批量导出
3. **📈 资金预测看板** - 7天预测曲线、缺口高亮、换汇方案
4. **✅ 换汇审批中心** - 方案列表、成本对比、多级审批
5. **✏️ 手动调整录入** - 大额预付/延期付款录入
6. **⚠️ 风险预警中心** - 汇率波动预警、对冲建议、敞口分析
7. **📑 月度报告管理** - 报告列表、PDF/Excel下载
...
- **UI组件**: 自定义响应式设计
- **图表**: Canvas 原生绘制（轻量无依赖）
- **数据交互**: Fetch API + RESTful

### 目录结构（新增Web相关）
```
fund_pool_system/
├── web_server.py              # Web服务器启动入口
├── src/api/
│   └── main.py               # FastAPI路由和接口实现
├── static/
│   ├── css/
│   │   └── style.css         # 全局样式
│   ├── js/
